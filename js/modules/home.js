...
if (typeof window.DREAM === 'undefined') {
    console.error('[HOME] DREAM object not found!');
}
    ...
    return `
      <div class="bento-grid animate-in">
        ...
        <div class="bento-card bento-medium">
          ...
          <div class="metric-value">${window.DREAM?.state?.networkStatus ? 'Online' : 'Offline'}</div>
          <div class="metric-trend up">${window.DREAM?.utils?.network?.type?.toUpperCase() || '4G'}</div>
        </div>
        
        <!-- Quick Actions -->
        <div class="bento-card bento-small">
          <div class="metric-label">⚡ Quick Actions</div>
          <button class="btn btn-primary" style="width: 100%; margin-top: 8px;" onclick="window.DREAM.load('qr-scanner')">
            Scan Now
          </button>
        </div>
